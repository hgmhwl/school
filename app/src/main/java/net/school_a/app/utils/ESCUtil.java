package net.school_a.app.utils;
...
public class ESCUtil {
    ...
    public static byte[] generateMockData() {
        try {
            byte[] ll = underlineWithOneDotWidthOn();
            byte[] lls = underlineWithTwoDotWidthOn();
            ...
            byte[][] cmdBytes = {ll, lls, title, nextLine, center, boldOn, fontSize2Big, Focus, boldOff, fontSize2Small,
    ...
    public static byte[] generateTiketData(PrintData data) {
        try {
            byte[] next2Line = ESCUtil.nextLine(2);
            String titleStr = "欢迎光临";
            byte[] title = titleStr.getBytes("gb2312");
            byte[] line = "---------------------------".getBytes("gb2312");

            byte[] boldOn = ESCUtil.boldOn();
            byte[] fontSize2Big = ESCUtil.fontSizeSetBig(1);
            byte[] center = ESCUtil.alignCenter();
            String lineStr = "商品    单价    数量    金额";
            String dataStr = "" + data.getName() + "    单价" + data.getPrice() + "    数量" + data.getNum() + "    " + data.getMoney();
            byte[] Focus = lineStr.getBytes("gb2312");
            byte[] boldOff = ESCUtil.boldOff();
            byte[] fontSize2Small = ESCUtil.fontSizeSetSmall(3);

            byte[] dataSerinum = dataStr.getBytes("gb2312");

            byte[] left = ESCUtil.alignLeft();
            // 订单号
            String carNo = "合计:   " + data.getTotal();
            byte[] noSerinum = carNo.getBytes("gb2312");

            left = ESCUtil.alignLeft();
            String desc1 = "谢谢惠顾，欢迎下次光临!";
            byte[] desc1Serinum = desc1.getBytes("gb2312");

            byte[] nextLine = ESCUtil.nextLine(1);
            byte[] desc4 = "微信关注  untrip2016  轻松赚钱".getBytes("gb2312");
            nextLine = ESCUtil.nextLine(1);
            byte[] desc5 = "网址：www.cnidian.com".getBytes("gb2312");

            byte[][] cmdBytes = {center, nextLine, line, nextLine, center, boldOn, title, nextLine, line, next2Line, Focus, next2Line, boldOff, fontSize2Small,
                    left, noSerinum, nextLine, dataSerinum, nextLine, nextLine, left, left, boldOn, boldOff,
                    next2Line, desc1Serinum, next2Line, left, nextLine, left, nextLine, nextLine, center, nextLine, left, nextLine, desc4, nextLine, desc5, nextLine, next2Line, next2Line};
